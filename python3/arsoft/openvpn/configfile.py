...
class ConfigFile(object):
            ...
            for comment in self._conf.comments:
                if comment.startswith('public-address'):
                    if type(value) in [list,tuple]:
                        self._conf.replace_comment(comment, ' public-address ' + ' '.join(value))
                    else:
        ...
        if self._conf is not None:
            if value:
                if isinstance(value, bool):
                    self._conf.set(section=None, key='auth-user-pass', value=None)
                else:
    ...
    def client_config_file(self, client_ccdfile=None):
        ...
        if client_ccdfile.disable_private_key_encryption is not None and client_ccdfile.disable_private_key_encryption:
            extra_comments.append('# disable-private-key-encryption')

        server_auth = ''
        for (plugin, plugin_params) in self.plugins:
            if 'openvpn-auth-pam.so' in plugin or 'openvpn-plugin-auth-pam.so' in plugin:
        ...
        if clientostype == 'windows':
            block_outside_dns_lines = 'block-outside-dns\npull "block-outside-dns"\n'

        remotes = []
        if type(self.public_address) in [list,tuple]:
            for addr in self.public_address:
                remotes.append('remote %s %i' % (addr, self.public_port))
        else:
            remotes.append('remote %s %i' % (self.public_address, self.public_port))
...
""" % {
    ...
    'dh_line': dh_line,
    'crl_line': crl_line,
    'block_outside_dns_lines': block_outside_dns_lines,
